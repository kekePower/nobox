...
interaction to one capability among several.
...
1. **Handshake.** The companion presents the agent's declared identity
   (harness name, purpose string) and the connecting UID from socket peer
   credentials.
...
Two standing indicators are WM-rendered and unspoofable: a persistent marker
while any session holds `input` or `capture`, and a visible marker (cursor
badge or frame tint) on the client currently receiving agent input.
...
content rectangle, valid while obscured, stamped with its geometry and
sequence number. `output.capture` captures a whole output and is deliberately
a separate named sensitivity, because full-screen pixels see everything.
...
trip. Arbitrary command execution is not a capability of this protocol.
...
event loop; any human input opens a configurable suppression window during
which agent input calls return a structured `interrupted` result and a
`human_activity` event fires. The protocol offers no way to contend with the
human for the pointer or keyboard — politeness is not delegated to the agent.
...
- No capability implies another; every request is validated against the grant
  inside the manager, not the companion.
...
- Human input preempts agent input; the kill chord is processed ahead of all
  agent traffic.
- Consent and status surfaces are WM-rendered and cannot be drawn, covered,
  or dismissed through the protocol.
- A dead, slow, or malicious companion never blocks or crashes the manager.
- Denied or interrupted requests return structured errors; nothing partially
  succeeds silently.
...
- Grant evaluation, scope filtering, sequence/coalescing rules, and
  sensitive-client visibility are pure policy and belong in `nobox-core` /
  `nobox-config` unit tests.
- Injection against live geometry, consent UI ownership, indicator rendering,
  kill-chord priority, and companion disconnect behavior belong in nested X11
  integration tests driving a real `nobox-agent` session.
