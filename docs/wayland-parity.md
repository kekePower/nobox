...
| `agent`: `enabled`, `socket`, `policy` | native | Controls the private explicit Wayland socket and stored-grant default. Discovery remains intentionally absent until the complete W8 acceptance flow passes. |
| `agent`: `suppression_ms`, `kill_chord` | intentionally unsupported | Reserved for the W8 consent and human-preemption tranche; no partial timing or emergency-control claim is made. |
| `agent.grants`: `label`, `executable`, `uid`, `capabilities`, `scope` | native | Verified peers receive only configured atoms that the Wayland backend has realized. Observation, management, desktop launch, and scene capture apply the shared scope/privacy/generation policy; later input atoms stay masked. |
