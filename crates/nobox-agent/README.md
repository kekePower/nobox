...
socket. It speaks MCP revision 2026-07-28 on stdio toward an agent harness and
the Agent Seat Protocol toward a window manager.
...
`server/discover` reports what the seat actually granted this session, so the
first question a host asks also answers "is the window manager there, and what
may I do".
